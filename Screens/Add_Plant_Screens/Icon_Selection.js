...
import { usePlant } from '../../Contexts/AddPlant_Context';
...
export function Icon_Selection(props) {
   ...
   const image_icons = useMemo(() => plant_icons.map((elem, i) => {
         return <Button key={i} 
            bg={ (pressed === i) ? "secondary.culturedGrey" : "transparent"} startIcon={<Image
            source={elem}
         ...
         />} onPress={() => {
            ...
            setContinue(true);
         }} _pressed={{bg: "secondary.culturedGrey"}}></Button>
      }), [pressed]);

   return <Box w="100%" h="60%" flexDirection="column" justifyContent="space-around" mb={5}>
   <Center>
      <Text fontSize="lg" fontWeight="bold">Choose an icon for your {Plant.plantName}</Text>
   </Center>
   <ScrollView>
